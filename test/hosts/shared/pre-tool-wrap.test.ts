...
describe("commandAlreadyWrapped", () => {
  it.each([
    ...
    ["token-glace wrap --raw -- pnpm test", true, "primary executable"],
    ["token-glace --full -- pnpm test", true, "primary executable full bypass"],
    ["FOO=1 tokenjuice --raw -- pnpm test", true, "leading assignment"],
    ["env FOO=1 tokenjuice --full -- pnpm test", true, "env assignment"],
    ["/usr/local/bin/tokenjuice wrap -- git status", true, "absolute POSIX path"],
    ["'/opt/Token Glace/bin/token-glace' wrap --full -- git status", true, "quoted primary path"],
    ["env -- /usr/local/bin/token-glace wrap --raw -- git status", true, "env absolute primary path"],
    ["token-glace.cmd wrap -- git status", true, "Windows primary shim"],
    ...
    ["token-glace doctor", false, "primary non-wrap subcommand"],
    ["env FOO=1 some-other-binary --raw -- git status", false, "env-wrapped unrecognized launcher"],
