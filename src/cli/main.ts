...
import { doctorAictlInstructions, installAictlInstructions, uninstallAictlInstructions } from "../hosts/aictl/index.js";
import { doctorAiMemoryProtocolMemory, installAiMemoryProtocolMemory, uninstallAiMemoryProtocolMemory } from "../hosts/ai-memory-protocol/index.js";
...
import { doctorBlackboxSkill, installBlackboxSkill, uninstallBlackboxSkill } from "../hosts/blackbox/index.js";
import { doctorBlocksSkill, installBlocksSkill, uninstallBlocksSkill } from "../hosts/blocks/index.js";
import { doctorClawdbotSkill, installClawdbotSkill, uninstallClawdbotSkill } from "../hosts/clawdbot/index.js";
...
import { doctorElyraSkill, installElyraSkill, uninstallElyraSkill } from "../hosts/elyra/index.js";
...
import { doctorStagewiseSkill, installStagewiseSkill, uninstallStagewiseSkill } from "../hosts/stagewise/index.js";
...
import { doctorTabbySystemPrompt, installTabbySystemPrompt, uninstallTabbySystemPrompt } from "../hosts/tabby/index.js";
...
export function decorateWrapInlineText(result: WrapResult["result"], raw: boolean): string {
  ...
  const footer = [
    ...
    result.rawRef?.id
      ? `Raw output is available locally: tokenjuice cat ${result.rawRef.id}`
      : "Full output: re-run with tokenjuice wrap --raw -- <command>",
  ].join("\n");
...
async function runInstall(args: ParsedArgs): Promise<number> {
  ...
  if (target === "aictl") {
    const result = await installAictlInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Instructions", value: result.instructionsPath },
      { label: "Beta", value: "AICTL.md project prompt guidance; aictl still owns command execution" },
      { label: "Verify", value: "tokenjuice doctor aictl" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("aictl", "instructions", details));
    return 0;
  }

  if (target === "ai-memory-protocol" || target === "aimemoryprotocol") {
    ...
    const memoryDir = dirname(dirname(result.memoryPath));
    const details = [
      { label: "Memory", value: result.memoryPath },
      { label: "Beta", value: "RST preference memory; AI Memory Protocol still owns recall and MCP serving" },
      { label: "Workspace", value: "requires memory init .memories --install first" },
      { label: "Rebuild", value: `MEMORY_DIR=${shellQuote(memoryDir)} memory rebuild` },
      { label: "Verify", value: "tokenjuice doctor ai-memory-protocol" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("ai-memory-protocol", "memory", details));
    return 0;
  }
  ...
  if (target === "baz") {
    const result = await installBazSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      ...
      { label: "Beta", value: "workspace skill guidance; Baz still owns review, fixer, and shell execution" },
      { label: "Verify", value: "tokenjuice doctor baz" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
  ...
  if (target === "blackbox") {
    const result = await installBlackboxSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "workspace skill guidance; Blackbox still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor blackbox" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("blackbox", "skill", details));
    return 0;
  }

  if (target === "blocks") {
    const result = await installBlocksSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "workspace skill guidance; Blocks still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor blocks" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("blocks", "skill", details));
    return 0;
  }

  if (target === "clawdbot") {
    const result = await installClawdbotSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "workspace skill guidance; Clawdbot still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor clawdbot" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("clawdbot", "skill", details));
    return 0;
  }
  ...
  if (target === "charlie") {
    const result = await installCharlieInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Instructions", value: result.instructionsPath },
      { label: "Beta", value: "AGENTS.md-based guidance; Charlie still owns command execution" },
      { label: "Verify", value: "tokenjuice doctor charlie" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("charlie", "instructions", details));
    return 0;
  }
  ...
  if (target === "codeant") {
    const result = await installCodeAntInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Instructions", value: result.instructionsPath },
      { label: "Beta", value: "instructions-based guidance; CodeAnt still owns review and fixes" },
      { label: "Verify", value: "tokenjuice doctor codeant" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("codeant", "instructions", details));
    return 0;
  }
  ...
  if (target === "coder-agents" || target === "coderagents") {
    const result = await installCoderAgentsSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "workspace skill guidance; Coder Agents still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor coder-agents" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("coder-agents", "skill", details));
    return 0;
  }
  ...
  if (target === "command-code" || target === "commandcode") {
    const result = await installCommandCodeHook(undefined, { local: args.local });
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      ...
      { label: "Verify", value: `tokenjuice doctor command-code${args.local ? " --local" : ""}` },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("command-code", "hook", details));
    return 0;
  }
  ...
  if (target === "elyra") {
    const result = await installElyraSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "project skill guidance; Elyra still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor elyra" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("elyra", "skill", details));
    return 0;
  }
  ...
  if (target === "coderabbit") {
    const result = await installCodeRabbitConfig();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Config", value: result.configPath },
      { label: "Beta", value: "path-instruction guidance; CodeRabbit still owns PR review and tools" },
      { label: "Verify", value: "tokenjuice doctor coderabbit" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("coderabbit", "review config", details));
    return 0;
  }
  ...
  if (target === "leanctl") {
    const result = await installLeanCtlInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Instructions", value: result.instructionsPath },
      { label: "Beta", value: "project instruction guidance; LeanCTL still owns command execution" },
      { label: "Verify", value: "tokenjuice doctor leanctl" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("leanctl", "instructions", details));
    return 0;
  }
  ...
  if (target === "stagewise") {
    const result = await installStagewiseSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "workspace skill guidance; Stagewise still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor stagewise" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("stagewise", "skill", details));
    return 0;
  }
  ...
  if (target === "novakit") {
    const result = await installNovaKitInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Instructions", value: result.instructionsPath },
      { label: "Beta", value: "NOVAKIT.md guidance; NovaKit still owns command execution" },
      { label: "Verify", value: "tokenjuice doctor novakit" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("novakit", "instructions", details));
    return 0;
  }
  ...
  if (target === "pi-go" || target === "pigo") {
    const result = await installPiGoSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Skill", value: result.skillPath },
      { label: "Beta", value: "project skill guidance; pi-go still owns shell execution" },
      { label: "Verify", value: "tokenjuice doctor pi-go" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("pi-go", "skill", details));
    return 0;
  }
  ...
  if (target === "tabby") {
    const result = await installTabbySystemPrompt();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
    ...
    const details = [
      { label: "Config", value: result.configPath },
      { label: "Beta", value: "system-prompt guidance; Tabby still owns command execution" },
      { label: "Verify", value: "tokenjuice doctor tabby" },
      { label: "Escape hatch", value: "tokenjuice wrap --raw -- <command>" },
    ];
    if (result.backupPath) {
      details.push({ label: "Backup", value: result.backupPath });
    }
    process.stdout.write(formatInstallSuccess("tabby", "system prompt", details));
    return 0;
  }
...
async function runUninstall(args: ParsedArgs): Promise<number> {
  ...
  if (target === "aictl") {
    const result = await uninstallAictlInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed aictl instructions: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`instructions path: ${result.instructionsPath}\n`);
    process.stdout.write("enable: tokenjuice install aictl\n");
    return 0;
  }

  if (target === "ai-memory-protocol" || target === "aimemoryprotocol") {
    const result = await uninstallAiMemoryProtocolMemory();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed ai-memory-protocol memory: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`memory path: ${result.memoryPath}\n`);
    process.stdout.write(`rebuild: MEMORY_DIR=${shellQuote(dirname(dirname(result.memoryPath)))} memory rebuild\n`);
    process.stdout.write("enable: tokenjuice install ai-memory-protocol\n");
    return 0;
  }
  ...
  if (target === "baz") {
    const result = await uninstallBazSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
  ...
  if (target === "blackbox") {
    const result = await uninstallBlackboxSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed blackbox skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install blackbox\n");
    return 0;
  }

  if (target === "blocks") {
    const result = await uninstallBlocksSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed blocks skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install blocks\n");
    return 0;
  }

  if (target === "clawdbot") {
    const result = await uninstallClawdbotSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed clawdbot skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install clawdbot\n");
    return 0;
  }
  ...
  if (target === "charlie") {
    const result = await uninstallCharlieInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed charlie instructions: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`instructions path: ${result.instructionsPath}\n`);
    process.stdout.write("enable: tokenjuice install charlie\n");
    return 0;
  }
  ...
  if (target === "codeant") {
    const result = await uninstallCodeAntInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed codeant instructions: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`instructions path: ${result.instructionsPath}\n`);
    process.stdout.write("enable: tokenjuice install codeant\n");
    return 0;
  }
  ...
  if (target === "coder-agents" || target === "coderagents") {
    const result = await uninstallCoderAgentsSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }
  ...
  if (target === "command-code" || target === "commandcode") {
    const result = await uninstallCommandCodeHook();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed command-code entries: ${result.removed}\n`);
    process.stdout.write(`settings path: ${result.settingsPath}\n`);
    process.stdout.write("enable: tokenjuice install command-code\n");
    return 0;
  }

  if (target === "pi-go" || target === "pigo") {
    const result = await uninstallPiGoSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed pi-go skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install pi-go\n");
    return 0;
  }
  ...
  if (target === "coderabbit") {
    const result = await uninstallCodeRabbitConfig();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed coderabbit review config: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`config path: ${result.configPath}\n`);
    process.stdout.write("enable: tokenjuice install coderabbit\n");
    return 0;
  }
  ...
  if (target === "leanctl") {
    const result = await uninstallLeanCtlInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed leanctl instructions: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`instructions path: ${result.instructionsPath}\n`);
    process.stdout.write("enable: tokenjuice install leanctl\n");
    return 0;
  }
  ...
  if (target === "stagewise") {
    const result = await uninstallStagewiseSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed stagewise skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install stagewise\n");
    return 0;
  }
  ...
  if (target === "novakit") {
    const result = await uninstallNovaKitInstructions();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed novakit instructions: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`instructions path: ${result.instructionsPath}\n`);
    process.stdout.write("enable: tokenjuice install novakit\n");
    return 0;
  }
  ...
  if (target === "tabby") {
    const result = await uninstallTabbySystemPrompt();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed tabby system prompt: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`config path: ${result.configPath}\n`);
    process.stdout.write("enable: tokenjuice install tabby\n");
    return 0;
  }
  ...
  if (target === "elyra") {
    const result = await uninstallElyraSkill();
    if (args.format === "json") {
      process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
      return 0;
    }

    process.stdout.write(`removed Elyra skill: ${result.removed ? "yes" : "no"}\n`);
    process.stdout.write(`skill path: ${result.skillPath}\n`);
    process.stdout.write("enable: tokenjuice install elyra\n");
    return 0;
  }
...
async function runDoctor(args: ParsedArgs): Promise<number> {
  ...
  if (args.positionals[0] === "aictl") {
    const report = await doctorAictlInstructions();
  ...
  if (args.positionals[0] === "ai-memory-protocol" || args.positionals[0] === "aimemoryprotocol") {
    const report = await doctorAiMemoryProtocolMemory();
    ...
    process.stdout.write(`memory path: ${report.memoryPath}\n`);
    process.stdout.write(`health: ${report.status}\n`);
    if (report.issues.length > 0) {
      process.stdout.write("issues:\n");
      for (const issue of report.issues) {
  ...
  if (args.positionals[0] === "baz") {
    const report = await doctorBazSkill();
  ...
  if (args.positionals[0] === "blackbox") {
    const report = await doctorBlackboxSkill();
  ...
  if (args.positionals[0] === "blocks") {
    const report = await doctorBlocksSkill();
  ...
  if (args.positionals[0] === "clawdbot") {
    const report = await doctorClawdbotSkill();
  ...
  if (args.positionals[0] === "charlie") {
    const report = await doctorCharlieInstructions();
  ...
  if (args.positionals[0] === "pi-go" || args.positionals[0] === "pigo") {
    const report = await doctorPiGoSkill();
  ...
  if (args.positionals[0] === "command-code" || args.positionals[0] === "commandcode") {
    const report = await doctorCommandCodeHook(undefined, { local: args.local });
  ...
  if (args.positionals[0] === "elyra") {
    const report = await doctorElyraSkill();
  ...
  if (args.positionals[0] === "coderabbit") {
    const report = await doctorCodeRabbitConfig();
  ...
  if (args.positionals[0] === "leanctl") {
    const report = await doctorLeanCtlInstructions();
  ...
  if (args.positionals[0] === "stagewise") {
    const report = await doctorStagewiseSkill();
  ...
  if (args.positionals[0] === "tabby") {
    const report = await doctorTabbySystemPrompt();
  ...
  if (args.positionals[0] === "codeant") {
    const report = await doctorCodeAntInstructions();
  ...
  if (args.positionals[0] === "coder-agents" || args.positionals[0] === "coderagents") {
    const report = await doctorCoderAgentsSkill();
...
async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  switch (args.command) {
    ...
    case "command-code-post-tool-use":
      return await runCommandCodePostToolUseHook(await readStdin(args.maxInputBytes));
