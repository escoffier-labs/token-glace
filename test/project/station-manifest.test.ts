...
describe("Brigade station manifest", () => {
  it("declares the supported Token Glace automation surfaces", async () => {
    const manifest = JSON.parse(await readFile("station.json", "utf8"));

    expect(manifest).toEqual({
      schema: "brigade.station.v1",
      name: "token-glace",
      station: "tokens",
      summary: "deterministic terminal output compaction and host hook health",
      lifecycle: "active",
      tools: [
        {
          name: "token-glace",
          command: "token-glace",
          summary: "compact terminal output and inspect host hook health",
          install: [
            "npm",
            "install",
            "-g",
