...
| OpenClaw | `tokenjuice install openclaw` | `~/.openclaw/workspace/.openclaw/extensions/tokenjuice-openclaw/` plus merged `plugins.allow`, `plugins.load.paths`, and `plugins.entries` in `~/.openclaw/openclaw.json` | Uses OpenClaw's synchronous `tool_result_persist` hook to substitute compacted output for the `exec` tool; installer backs up `openclaw.json` before merging; `tokenjuice install openclaw --local` bundles the runtime from the current repo source; `OPENCLAW_CONFIG_PATH` is honored for non-default config layouts. **Note:** the OpenClaw hook is synchronous, so user and project rule overlays (`~/.config/tokenjuice/rules`, `<cwd>/.tokenjuice/rules`) are not applied on this host. Only the bundled built-in rule set is loaded. Callers that need custom rules on OpenClaw should use `tokenjuice wrap` from inside the `exec` command instead. |
